...
-- Runs every tick. Throttled by vai_itemscan; when the scan fires it looks for
-- nearby items worth collecting and sets mode 6 to walk toward the first one found.
function fai_collect(id)
    -- Early-out before touching the counter so mode-6 bots don't burn their budget
    if vai_mode[id] == 6 then return end
    if p(id, "team") == 1 and vai_set_gm == 4 then return end  -- zombies skip
    ...
    for i = 1, #items do
        ...
        if ix ~= px or iy ~= py then
            ...
            elseif slot == 5 then
                collect = itype == WPN_BOMB and team == 1  -- only T picks up the bomb
            elseif slot == 0 then
...
-- MODE 7: follow a teammate (stored in vai_smode) and roam nearby when close.
--
-- State while in mode 7:
--   vai_smode[id]       = leader player ID
--   vai_timer[id]       = roam ticks remaining (0 means actively following)
--   vai_followangle[id] = current roam direction in degrees
--
-- vai_followangle is kept separate from vai_destx so navigation state isn't
-- corrupted when the bot exits follow mode
function fai_follow(id)
    ...
    if vai_timer[id] > 0 then
        ...
        if ai_move(id, vai_followangle[id], 1) == 0 then
            ...
            -- Resume following if the leader has moved away
    ...
    -- Actively following; switch to roam once we catch up
